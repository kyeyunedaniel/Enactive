// resources/js/Pages/Onboarding.jsx
...
// --- Step-specific UI Components (Moved outside the main component) ---

const Step1_Profile = ({ profileData, handleProfileChange, profilePhotoPreview, fileInputRef, handleFileChange }) => (
        ...
        <div className="bg-white rounded-2xl shadow-md p-8 grid md:grid-cols-3 gap-8 items-start">
            ...
            <div className="md:col-span-2 space-y-6">
                <div>
                    ...
                    <Input id="name" value={profileData.name} onChange={handleProfileChange} placeholder="Your public name"/>
                </div>
                <div>
                    <label htmlFor="about" className="block text-sm font-medium text-gray-700 mb-1">About</label>
                    <Textarea id="about" value={profileData.about} onChange={handleProfileChange} rows={5} placeholder="Tell your supporters about yourself"/>
                </div>
                <div>
                    <label htmlFor="socialLink" className="block text-sm font-medium text-gray-700 mb-1">Website or social link</label>
                    <Input id="socialLink" value={profileData.socialLink} onChange={handleProfileChange} placeholder="https://"/>
                </div>
            </div>
        </div>
    </div>
...
const Step2_Payouts = ({ payoutData, handlePayoutChange, countries }) => (
    <div className="w-full max-w-lg text-center">
        ...
        <div className="bg-white rounded-2xl shadow-md p-8 mt-8">
            <div className="relative">
                ...
                <select id="country" value={payoutData.country} onChange={handlePayoutChange} className="w-full pl-12 pr-4 py-3 bg-gray-100 border border-transparent rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white">
                    {countries.map(c => <option key={c.code} value={c.code} disabled={c.disabled}>{c.name}</option>)}
                </select>
            </div>
...
const Step3_MobileMoney = ({ payoutData, handlePayoutChange, isVerifying }) => (
            ...
            {!isVerifying ? (
                <>
                    <div>
                        ...
                        <Input id="phoneNumber" value={payoutData.phoneNumber} onChange={handlePayoutChange} placeholder="e.g., 0772123456" type="tel"/>
                    </div>
                    <div>
                        <label htmlFor="registeredName" className="block text-sm font-medium text-gray-700 mb-1">Registered Mobile Money Name</label>
                        <Input id="registeredName" value={payoutData.registeredName} onChange={handlePayoutChange} placeholder="e.g., John Doe"/>
                    </div>
                </>
            ) : (
                <div>
                    ...
                    <Input id="verificationCode" value={payoutData.verificationCode} onChange={handlePayoutChange} placeholder="______" maxLength="6" type="text" pattern="\d*"/>
                </div>
            )}
        </div>
    </div>
...
export default function Onboarding({ auth }) {
    ...
    const handleVerify = (e) => {
        e.preventDefault();
        // Here you would call your backend to send an SMS
        console.log("Simulating sending verification code to:", payoutData.phoneNumber);
        setIsVerifying(true);
    };
    ...
    const handleComplete = (e) => {
        ...
        // Here you would post all data to your backend endpoint for final submission
        // using Inertia's post method.
        // router.post(route('onboarding.complete'), { profileData, payoutData });
        
        console.log("Submitting all data...", { profileData, payoutData });
        setTimeout(() => {
    ...
    const handleSkipPayouts = () => {
        // You might want to post just the profile data here before skipping
        router.get(route('dashboard.home'));
    }
                ...
                <footer className="w-full px-4 sm:px-6 py-6 bg-white border-t border-gray-200">
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="flex items-center justify-end mt-6">
                            {step === 2 && (
                                <button onClick={handleSkipPayouts} className="font-semibold text-gray-600 hover:text-gray-900 mr-6">
                                    Skip this step
                                </button>
                            )}
                            {step === 3 && !isVerifying && (
                            ...
                            {step === 3 && isVerifying && (
                                <Button onClick={handleComplete} disabled={!isVerificationValid || isSubmitting}>
                                    {isSubmitting ? <ArrowPathIcon className="w-5 h-5 animate-spin mx-auto" /> : 'Complete Setup'}
                                </Button>
                            )}
                            {step < 3 && (
                                <Button onClick={() => setStep(step + 1)} disabled={(step === 1 && !isStep1Valid) || (step === 2 && payoutData.country !== 'UG')}>
                                    Next
                                </Button>
                            )}
                        </div>
