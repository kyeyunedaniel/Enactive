...
const Card = ({ children, className = '', title, description, actionText, ActionIcon, actionHref = "#", onActionClick }) => (
...
const StreamAlertPreview = () => (
    <div className="relative h-40 w-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex items-center justify-center p-4 overflow-hidden group">
        <div className="relative w-64 h-20">
            ...
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-8 bg-green-700 flex items-center justify-center transform -skew-x-[20deg]">
                <p className="text-white text-xs font-bold tracking-wider transform skew-x-[20deg]">NEW SUPPORTER</p>
            </div>
            {/* White banner part */}
            <div className="absolute top-5 left-0 right-0 h-12 bg-white flex items-center justify-center border border-gray-200">
...
// Enhanced QR Code with green accents - Now with actual QR code
const QrCodeWithLogo = ({ qrCode, qrCodeUrl }) => {
    const qrRef = useRef(null);

    // Safe type checking
    const isDataUri = typeof qrCode === 'string' && qrCode.startsWith('data:image/svg+xml');
    
    console.log('QR Code in component:', { 
        type: typeof qrCode, 
        value: qrCode,
        isDataUri 
    });
            ...
            {isDataUri ? (
                <div className="relative w-32 h-32 bg-white p-2 rounded-lg shadow-sm">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="w-8 h-8 bg-green-600 rounded flex items-center justify-center p-1">
                            <svg className="w-full h-full text-white" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M18.5 3H6c-1.93 0-3.5 1.57-3.5 3.5V11c0 1.93 1.57 3.5 3.5 3.5h7c1.93 0 3.5-1.57 3.5-3.5V9h2.5c1.38 0 2.5-1.12 2.5-2.5S23.38 4 22 4h-2.5V3.5C19.5 3.22 18.78 3 18.5 3zM16.5 11c0 .83-.67 1.5-1.5 1.5h-7c-.83 0-1.5-.67-1.5-1.5V6.5C6.5 5.67 7.17 5 8 5h7c.83 0 1.5.67 1.5 1.5V11zM20 16h-2v4h2c1.1 0 2-.9 2-2s-.9-2-2-2z"/>
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative w-32 h-32">
                    {/* Fallback QR Code SVG background */}
                    ...
                    {/* Debug info */}
                    {qrCode && (
                        <div className="absolute inset-0 flex items-center justify-center bg-yellow-50 bg-opacity-90 rounded">
                            <p className="text-xs text-yellow-800 text-center p-2">
                                QR Code Type: {typeof qrCode}<br/>
                                Expected: data URI string
                            </p>
                        </div>
                    )}
                </div>
...
export default function ButtonsAndGraphics({ auth, qrCode, qrCodeUrl }) {
    // Debug the props
    console.log('Page props received:', { 
        qrCode, 
    ...
    const downloadQRCode = () => {
        // Safe type checking
        const isValidQrCode = typeof qrCode === 'string' && qrCode.startsWith('data:image/svg+xml');
        
        if (!isValidQrCode) {
            console.error('No valid QR code data available. Type:', typeof qrCode, 'Value:', qrCode);
            alert('QR code is not available for download. Please try again.');
            return;
        }
        ...
        try {
            ...
            // Decode base64 to get the SVG string
            ...
            link.href = url;
            const fileName = `qr-code-${auth.user?.public_url_name || 'support'}.svg`;
            ...
            <div className="space-y-12">
                ...
                {qrCode && typeof qrCode !== 'string' && (
                    <div className="bg-yellow-100 border border-yellow-400 rounded p-4">
                        <p className="text-yellow-800 text-sm">
                            <strong>Debug Info:</strong> QR code is type "{typeof qrCode}" but expected "string".<br/>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Card 
                        ...
                        onActionClick={downloadQRCode}
                    >
                        <QrCodeWithLogo qrCode={qrCode} qrCodeUrl={qrCodeUrl} />
                    </Card>
                </div>
